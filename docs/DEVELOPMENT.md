...
## Longer event and explicit PBC visualization

`run_impact_event.py` reruns the saved initial neutral impact in a 100 A-tall fully periodic cell for 2 ps, with a top-boundary guard and incremental trajectory/CSV output. `report_impact_event.py` checks the energy log and a sustained carbon-component departure criterion. Its results live separately in `results/long-md/interface-event-gpu/`.

`render_periodic_md.py` displays the actual cell vectors, faded periodic image atoms and bonds across cell faces. The overview includes vacuum; the interface close-up follows the original projectile carbon. `render_atomistic.py` delegates to this renderer. Comparisons use physical timestamps in render manifests, and the main README uses the extended impact and existing 2 ps crystals. See `docs/VISUALIZATION.md`.
